/**
 * Resolve variáveis em strings de respostas rápidas, mensagens e fluxos.
 * Tokens suportados:
 *   {{nome}}            -> contact.name
 *   {{primeiro_nome}}   -> primeiro token de contact.name
 *   {{telefone}}        -> contact.phone
 *   {{email}}           -> contact.email
 *   {{empresa}}         -> options.companyName ?? contact.company
...
export type ResolveOptions = {
  /** Sobrescreve {{empresa}} com o nome global da empresa quando o contato não tem `company`. */
...
export function resolveContactVariables(
  template: string,
  contact: VariableContact | null | undefined,
  ...
  return template.replace(TOKEN_RE, (match, raw: string) => {
    ...
    if (key === "empresa") return contact?.company ?? options.companyName ?? match;
